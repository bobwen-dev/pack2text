...
pub fn pack_directories(
    ...
    pack_files(&all_files)
}

fn pack_files(files: &[CollectedFile]) -> Result<PackResult> {
...
mod tests {
    ...
    fn pack_single_file() {
        ...
        let root = d.path();
        let result = pack_directories(&[root], None, None, None).unwrap();
    ...
    fn pack_empty_dir_fails() {
        ...
        let root = d.path();
        let result = pack_directories(&[root], None, None, None);
    ...
    fn pack_not_a_dir_fails() {
        ...
        let result = pack_directories(&[&file], None, None, None);
        assert!(result.is_err());
    }
    ...
    fn pack_selected_roundtrip_byte_exact() {
        ...
        let result = pack_selected(&[&a], None, None, None).unwrap();
        assert_eq!(result.file_count, 1);
